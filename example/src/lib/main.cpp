...
void read_overlapping_instance(xml_node<>* node) {
    ...
    const std::string image_path = directory_path + "samples/" + name + ".png";
    ...
    for (unsigned i = 0; i < screenshots; i++) {
        for (unsigned test = 0; test < 10; test++) {
            ...
            if (success.has_value()) {
                std::string file_path = directory_path + "results\\" + name + to_string(i) + ".png";
                std::cout << "writing file to " << file_path << std::endl;
                write_image_png(file_path, *success);
...
void read_simpletiled_instance(xml_node<>* node,
    ...
    ifstream config_file(directory_path + "samples/" + name + "/data.xml");
    ...
    for (unsigned test = 0; test < 10; test++) {
        ...
        std::optional<Array2D<Color>> success = wfc.run();
        if (success.has_value()) {
            write_image_png(directory_path + "results/" + name + "_" + subset + ".png", *success);
            cout << name << " finished!" << endl;
            break;
        }
        else {
...
void read_config_file(const string& config_path) noexcept {
    ...
    string dir_path = directory_path + "samples";
